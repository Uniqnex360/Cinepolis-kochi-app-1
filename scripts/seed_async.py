...
CINEMAS = [{'name': 'Cinepolis Centre Square', 'screens': 2}]
CITY = 'Kochi'
...
async def main():
    ...
    async with S() as s:
        ...
        u = (await s.execute(select(User).where(User.email=='demo@kochi1.local'))).scalar_one_or_none()
        if not u:
            u = User(id=uuid.uuid4(), email='demo@kochi1.local', password_hash=pwd.hash('demo1234'), role='PARTNER')
